...
export default function Favoritar({ artistId, artistName, monthlyListeners, worldRank, followers }) {
    ...
    useEffect(() => {
        const verificarFavorito = async () => {
            try {
                const response = await axios.get(`https://projeto-2-backend-guedesluigi.onrender.com/api/spotify/${artistId}/`);
    ...
    const definirFavorito = () => {
        ...
        if (novoEstado) {
            axios
            .post(`https://projeto-2-backend-guedesluigi.onrender.com/api/spotify/${artistId}/`, { name: artistName, monthlyListeners: monthlyListeners, worldRank: worldRank, followers: followers, favoritado: novoEstado })
            .then(() => {
                console.log("Artista favoritado!");
            });
        } else {
            axios
            .delete(`https://projeto-2-backend-guedesluigi.onrender.com/api/spotify/${artistId}/`)
            .then(() => {
